...
| `apps/web/FE_PHASE.md` | Frontend Dev | *Created by Frontend Dev.* The frontend's own phase tracker — `Phase.md` lives on the backend branch and is not visible to them. |
| `Backend_Roadmap.md` + `Antigravity_Prompts.md` | Team Lead | Step-by-step backend build with Antigravity. |
| `Frontend_Roadmap.md` + `Antigravity_Prompts_Frontend.md` | Team Lead | Step-by-step frontend build with Antigravity. |
| `Integration_Guide.md` | Team Lead | 🔴 Branch model, path ownership, contract sync, merge procedure. **Both devs read this first.** |
...
### 7.0 🔴 Three branches, disjoint paths

This is one repository with three branches. `main` holds the documents,
`contract/` and shared config. `backend` holds Python. `frontend` holds
TypeScript. **No file is created on both.**

Both devs merge `main` daily. The backend publishes `contract/openapi.yaml` to
`main`; the frontend consumes it from there. Neither branch is ever merged into
the other — both merge into `main` at the end.

`Integration_Guide.md` is the full specification: the path-ownership map, the
contract-sync ritual, the four integration checkpoints and the merge procedure.
🔴 **Both devs read it before writing any code.**
