...
🔴 **Never CREATE a file the other branch owns either.** This repository has
three branches with deliberately disjoint path sets — `Integration_Guide.md` §2
is the map. A frontend agent creating a root `Dockerfile`, or a backend agent
creating a `package.json`, produces a merge conflict weeks later that nobody
remembers the cause of. If a task seems to need it, stop and ask.
